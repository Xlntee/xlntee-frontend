...
const RegistrationPage: FC = () => {
  ...
  const [state, setState] = useState<RegistrationState>(
    RegistrationState.SIGN_UP
  );

  const [signUp, { isLoading: isSignUpLoading }] = useSignUpMutation();
  const [verifyEmail, { isLoading: isVerificationLoading }] =
    useVerifyEmailMutation();
  const [acceptPolicy, { isLoading: isPolicyAcceptionLoading }] =
    useAcceptPolicyMutation();

  const onSubmit = handleSubmit(async (data) => {
    console.log(data);

    if (isSignUpLoading || isVerificationLoading || isPolicyAcceptionLoading)
      return;
