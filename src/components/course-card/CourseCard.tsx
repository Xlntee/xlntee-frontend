...
const CourseCard: FC<IProps> = ({title = 'Основи шрифта і як його правильно построїти',
                    size = 'small',
    ...
    return (
        <div className={`${size === 'small' ? 'course-card' : 'course-card--large'} `}>
            ...
            <div className='course-card__bottom'>
                ...
                <div className='course-card__text-block'>
                    ...
                    <div className='course-card__teacher-date-block'>
                        <a className='course-card__teacherBtn' href='#' type='button'>{teacherName}</a>
                        <span className='course-card__date'>{date}</span>
                    </div>
                        <RatingComponent size={size} rating={rating} feedbackCount={feedbackCount}/>
                        <Price size={size} isFree={false} startPrice={startPrice} discountPrice={discountPrice} persentage={persentage} />
                </div>
            </div>
        </div>
    )
